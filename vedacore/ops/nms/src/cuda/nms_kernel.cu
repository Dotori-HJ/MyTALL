...
__global__ void nms_kernel(const int n_segments, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < row_size) {
    ...
    const int col_blocks = at::ceil_div(n_segments, threadsPerBlock);
...
at::Tensor nms_cuda_forward(const at::Tensor segments, float nms_overlap_thresh) {
  ...
  const int col_blocks = at::ceil_div(segments_num, threadsPerBlock);
  ...
  mask_dev = (unsigned long long*) AT_CUDA_CHECK(c10::cuda::CUDACachingAllocator::raw_alloc(segments_num * col_blocks * sizeof(unsigned long long)));
